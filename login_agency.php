...
if(isset($_REQUEST['btn_login']))	//button name is "btn_login" 
{
	...
	else
	{
		try
		{
			$select_stmt=$db->prepare("SELECT * FROM {$agency_tb} WHERE agency_name=:uname OR agency_mail=:uemail"); //sql select query
			$select_stmt->execute(array(':uname'=>$username, ':uemail'=>$email));	//execute query with bind parameter
			$row=$select_stmt->fetch(PDO::FETCH_ASSOC);
			
			if($select_stmt->rowCount() > 0)	//check condition database record greater zero after continue
			{
				if($username==$row["agency_name"] OR $email==$row["agency_mail"]) //check condition user taypable "username or email" are both match from database "username or email" after continue
				{
					//if(password_verify($password, $row["agency_password"])) //check condition user taypable "password" are match from database "password" using password_verify() after continue
					if($password==$row["agency_password"])
					{
						$_SESSION["user_login"] = $row["agency_id"];	//session name is "user_login"
						$_SESSION["user_type"] = "agency";				//session type is "client"
