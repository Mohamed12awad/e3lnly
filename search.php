...
<!-- <h3 class="font-weight-bold">Billboards</h3> -->
  ...
  count(tot_id) from (select billboard_id id from {$billboard_tb}
union all
select channel_id id from {$television_tb}
union all
select websitead_id id from {$website_tb}) as tot_id")->fetchColumn();
...
$nextlink = ($page < $pages) ? '<a href="?page=' . ($page + 1) . '" title="Next page">&rsaquo;</a> <a href="?page=' . $pages . '" title="Last page">&raquo;</a>' : '<span class="disabled">&rsaquo;</span> <span class="disabled">&raquo;</span>';
   ...
   ";
      ...
      $select_stmt_bill = $db->prepare($select);
      $select_stmt_bill->execute(array(':limit'=>$limit,':offset'=>$offset)); //execute query 
      if($select_stmt_bill->rowCount() > 0)	//check condition database record greater zero after continue
			{
        ?>
      <div class="row">
      <div class="col-12">
      <h2 class="my-3">Billboards</h2>
      <?php 


        // $row22 = $select_stmt_bill->fetch(PDO::FETCH_ASSOC);
        //     if(empty($row22['map_location'])){
              
        //         echo "<p class='text-center'>Sorry No Remaining Billboards Available</p>";
        //       }
      ...
      <?php
      while($row=$select_stmt_bill->fetch(PDO::FETCH_ASSOC))
      {

          $Channel_type = "Billboard";
          $bg_img = "https://i7.uihere.com/icons/874/1005/107/billboard-a2c03a1f93da1e5a8814ff11890017e7.png";
            ...
            <div class="card bg-light mx-auto" style="max-width:250px">
              ...
              <div class="card-body text-capitalize">
                ...
                <h6 class="card-subtitle mb-2 text-muted">status: <?php echo $row['status']?> </h6>
                  <?php if(!empty($row['map_location'])){ ?><p class="card-text">Location: <?php echo $row['map_location'] ?></p> <?php } ?>
  ...
  <?php
      ...
      ?>
<?php
      $select_stmt_tv = $db->prepare($select);
      $select_stmt_tv->execute(array(':limit'=>$limit,':offset'=>$offset)); //execute query 
      if($select_stmt_tv->rowCount() > 0)	//check condition database record greater zero after continue
			{
        ?>
      <div class="row">
      <h2 class="col-12 my-3">Television ADs</h2>
      
      <?php

      while($row=$select_stmt_tv->fetch(PDO::FETCH_ASSOC))
      {

          $Channel_type = "TV AD";
          $bg_img = "./assets/imgs/download.png";

        if(!empty($row['tv_channel_name'])){
        ?>
        <div class="col-xl-3 col-md-6 mb-4">
            <div class="card bg-light mx-auto" style="max-width:250px">
              <img src=<?php echo $bg_img ?> class="img-fluid card-img-top" alt="...">
              ...
              <div class="card-body text-capitalize">
                <h5 class="card-title font-weight-bold"><?php echo $Channel_type?></h5>
                <h6 class="card-subtitle mb-2 text-muted">status: <?php echo $row['status']?> </h6>
                  <?php if(!empty($row['tv_channel_name'])){ ?><p class="card-text">channel name: <?php echo $row['tv_channel_name'] ?></p> <?php } ?>
                <p class="card-text">end of contract: <?php echo $row['end_of_contract_date'] ?></p>
